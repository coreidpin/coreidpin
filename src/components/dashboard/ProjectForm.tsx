    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            <Label className="text-black font-medium">Project Media</Label>
            <MediaUpload
              value={mediaUrls || []}
              onChange={(urls) => setValue('media_urls', urls, { shouldValidate: true })}
              maxFiles={5}
              acceptedTypes={['image/*', '.pdf', '.doc', '.docx']}
              maxSize={10}
            />
            {errors.media_urls && <p className="text-sm text-red-600">{errors.media_urls.message}</p>}
          </div>
